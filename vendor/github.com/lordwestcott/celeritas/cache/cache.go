...
type Cache interface {
	...
	Set(string, interface{}, ...int) error //...int -> optional expiration time.
	Forget(string) error
...
func (c *RedisCache) Set(str string, value interface{}, ttlSec ...int) error {
	key := fmt.Sprintf("%s:%s", c.Prefix, str)
	conn := c.Conn.Get()
	defer conn.Close()
	...
	if len(ttlSec) > 0 {
		_, err := conn.Do("SETEX", key, ttlSec[0], string(encoded))
		if err != nil {
			return err
		}
	} else {
